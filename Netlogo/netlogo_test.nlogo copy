globals [angleStart degreesDrawnPerIteration was-mouse-down? presents sounds]
extensions [sound]
breed [ornaments ornament]
breed [gifts gift-box]
breed [snow flakes]

to setupTriangles [Cx Cy times]
  cro 10 [
    setxy Cx Cy
  ]
  let x 0
  while [x < 10] [
    ask turtle (x + (times * 10)) [
      set shape "triangle"
      set size 10 - x
      set heading 10 - x * 2
      set color red__orange____yellow___green____blue____indigo____violet____pink + x ;; the long name allows the user to see which color they are picking in the slider
      set x x + 1
;; please note that this can be done without the use of any variables through setting the features of the variables manually or using lists (among other things)
    ]
  ]
end

to setupTriangleArray
  ca
  ask patches [
    set pcolor white
  ]
  setupTriangles 0 0 0
  setupTriangles -8 8 1
  setupTriangles 8 -8 2
  setupTriangles 8 8  3
  setupTriangles -8 -8 4
  ;; this can also be done manually (using a helper function/local variables is not required it's just more efficient).
end
...
to planet
  cro 1 [
    ...
    wait 0.3
    set heading 0
    repeat (10 * orbit) [fd .1 wait 0.01]
  ]
end
...
to planet_moon
    ...
    wait 0.3
    set heading 0
    repeat (10 * (orbit + 2)) [fd .1 wait 0.01]
  ]
end

to moveInCircle [Cxcor Cycor r turtle#]
  ask turtle turtle# [
    let a 0
    loop [
      if a  > 360 [stop]
      setxy (Cxcor + ((sin a) * r)) (Cycor + ((cos a) * r))
      wait 0.005
      set a (a + 1)
    ]
    pu
...
to drawSegOfCIrcle [Cxcor Cycor r arc-length angle-start turtle#]
  ask turtle turtle# [
  pu
  set pen-size 2
  let a angle-start
    while [a  < arc-length + angle-start] [
    pd
    setxy (Cxcor + ((sin a) * r)) (Cycor + ((cos a) * r))
    set a a  + 1
  ]
...
to setup
  ...
  planet_moon
  set angleStart 1
  set degreesDrawnPerIteration 1
end

to planet_orbit
  drawSegOfCIrcle  0 0 orbit degreesDrawnPerIteration angleStart 1
  set angleStart angleStart + degreesDrawnPerIteration
  if remainder angleStart 10 = 0 [
        moveInCircle [xcor] of turtle 1 [ycor] of turtle 1 2 2
      ]
end

to make-a-tree
  ca
  set-default-shape turtles "drop"
  create-ornaments 10 [
    set size 2
    set heading 0
  ]
...
to go
  set sounds [ "Orchestral Harp" "Choir"  "Bird Tweet"]
  set presents [ "ball baseball" "ball basketball" "ball football" "ball tennis" "bike" "computer workstation" "dollar bill"]
  if mouse-down? and not was-mouse-down? [
    let closestOrnament min-one-of ornaments [distancexy mouse-xcor mouse-ycor]
      ask closestOrnament [
        if distancexy mouse-xcor mouse-ycor < (size / 2)  [
          sound:play-note one-of sounds 60 64 2
          repeat 2 [
    ...
    let clostestGift min-one-of gifts [distancexy mouse-xcor mouse-ycor]
    ask clostestGift [
      if distancexy mouse-xcor mouse-ycor < (size / 2)  [
        set shape one-of presents
        wait 1
        set shape "git-box"
      ]
    ]
...
421
67
red__orange____yellow___green____blue____indigo____violet____pink
red__orange____yellow___green____blue____indigo____violet____pink
9
139
39.0
...
887
325
Extra Credit (Advanced Netlogo Option)
11
0.0
1
...
220
393
For further informtion view the info tab (for extra credit as well).
15
0.0
1
...
886
496
Note: I have noticed that the sound extension occasionally lags/ does odd things, if this occurs just reopen the file.
11
0.0
1
...
The model represents a collection of gem type-shapes spinning to the right. 

**Model 1** includes:

  * 5 collections of turtles (look closely at how they are constructed to create the illusion), with a color gradient. 
  * A collection of turtles in the middle, and a collection in each corner. 

## HOW IT WORKS

**In function _setup triangles_:**

  * A series of triangle shaped turtles are used to construct a gem type shape (feel free to set this up manually or use an algorithim, which is more efficient, but more complex). 

**In function _spin_:**  

  * All of the turtles spin to the right. 

### Colors:

  * Make sure to use the color swatches feature under tools to observe patterns in color.
  * Naming the color variable as I have is NOT required, it is mostly for the user to be able to visualize the color change. 
  * Ensure that the color variable has no effect throughout the spin function, and only has an effect if _setup triangles_ is pressed.

# Exercise 2
## WHAT IS IT?

The model represents a extremely simple solar solar system.

**Model 2** includes:

  * A turtle in the middle representing the sun. 
  * A turtle that represents a planet.
  * A turtle that represents a moon.

## HOW IT WORKS

  * The sun stays static

**In function _setup_:**

  * The sun is created. 
  * The planet and moon are created, and moved to their respective positions (which are determined using the orbit variable).

**In function _planet_orbit_:**

  * The planet orbits around the sun (drawing a circle), with a raduis of "orbit."
  * The moon orbits around the planet, as every 10 degrees of the circle are drawn. 
	* The "orbit" size should be able to change at any point.
	* The user should be able to unpress the planet-orbit button AT ANY TIME and have it stop and then start again (from where it just left off) when the user presses the button again.

# Extra Credit 

## WHAT IS IT?

The model represents a christmas tree (made using links and turtles), includes (openable) gifts under the tree, and snow in the background. 

## HOW IT WORKS

**In function _Draw the Tree_:**

  * A christmas tree is drawn with ornaments as nodes (drops, and a star on top), gifts are placed under the tree, and snow is created. 

**In function _go_:**

 * If you press on a **ornament**:
	* Music will play
	* The ornament will change color, and turn briefly. 
	* Following the click (immediately after), the ornament will move to where the mouse is dragged (where the mouse is held down). 

  * If you press on a **gift**:
	* The gift in the gift box will be briefly revealed

  * The snow will fall continously, until a click occurs and it stops briefly. 

## Credits

Elizabeth Paperno, Period 4, Netlogo Test
@#$#@#$#@
default
true
0
...
Circle -7500403 false false 24 144 102
Circle -16777216 false false 174 144 102
Circle -7500403 false false 177 148 95
...
Polygon -11221820 true false 187 92 198 92 208 97 217 100 231 93 231 84 216 82 201 83 184 85
Polygon -7500403 true false 71 86 98 93 101 85 74 81
Rectangle -16777216 true false 75 75 75 90
Polygon -7500403 true false 70 87 70 72 78 71 78 89
...
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75
Polygon -2674135 false false 111 82
Polygon -1184463 true false 78 48 210 108 235 96 97 37 79 47
Polygon -2674135 true false 71 218
Polygon -1184463 true false 71 97 194 33 219 43 97 110 72 97 83 99
Polygon -1184463 true false 76 98 94 107 79 101 77 99
Polygon -2674135 true false 215 109
Polygon -1184463 true false 211 108 212 256 238 244 234 92 212 106
Polygon -1184463 true false 94 112 91 258 68 249 70 97 98 111
